...
bootstrap_vcov <- function(obj, data = NULL, B = NULL, block = NULL) {
  ...
  blocks <- NULL
  if (!is.null(obj$coords) && isTRUE(obj$coords$mode == "point") && block > 1L) {
  ...
  list(coef = Vcoef, gate = Vgate, all = Vall, method = "boot",
       note = sprintf("%s bootstrap, %d replicates",
                      if (is.null(blocks)) "xy-pairs" else "spatial-block", nrow(draws)))
}
